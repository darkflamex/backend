...
public class AuthenticationTokenImpl extends AbstractAuthenticationToken {
    ...
    public void authenticate() {
        if (getDetails() != null && getDetails() instanceof ApiToken && !((ApiToken) getDetails()).hasExpired()) {
    ...
    public String getHash() {
        return DigestUtils.md5DigestAsHex(String.format("%s_%d", username, ((ApiToken) getDetails()).getCreated().getTime()).getBytes());
    }
